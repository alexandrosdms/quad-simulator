classdef QuadPlot < handle
    ...
    methods
        ...
        function Q = QuadPlot(qn, state, wingspan, height, color, max_iter, h_3d)
            ...
            if nargin < 7, h_3d = gca; end % if subplot not defined, return current axes
            Q.h_3d = h_3d; % otherwise use the axis defines by h_3d
            hold(Q.h_3d, 'on') % in order to change graphics using set() we need to hold on
            Q.h_pos_hist = plot3(Q.h_3d, Q.state(1), Q.state(2), Q.state(3), 'rx');
