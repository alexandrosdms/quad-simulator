...
quadcolors = lines(1); % defines the colors of the quadrotor
...
stop_pos  = des_stop(1);
x0        = des_start;
xtraj     = nan(max_iter*nstep, length(x0)); % initialize to NaN values
...
for iter = 1:max_iter
  ...
  tic; % Recording elapsed time: Start Stopwatch
  % Initialize quad plot
  if iter == 1
    subplot(1,2,1);
    quad_state = simStateToQuadState(x0);
    QP = QuadPlot(1, quad_state, params.arm_length, 0.05, quadcolors(1,:), max_iter, h_3d); % Constructor
